...
[ExecuteInEditMode]
public class ArenaPart1stRow : MonoBehaviour {
	...
	public GameObject Row3BackWall;
	...
	public GameObject StairsEntrance;
	...
	public GameObject Row2Stairs;
	public GameObject Row2BaseBottom;
	public GameObject Row2BaseTop;
	...
	public GameObject Row3Stairs;
	public GameObject Row3BaseMiddle;
	public GameObject Row3BaseBottom;
	public GameObject Row3BaseTop;
	public GameObject Row3Tunnel;
	...
	void CleanUpEverything(){
		...
		if (!Row2.activeInHierarchy){
			...
			if (!Row2Stairs.activeInHierarchy)DestroyImmediate(Row2Stairs);
			if (!Row2BaseBottom.activeInHierarchy)DestroyImmediate(Row2BaseBottom);
			if (!Row2BaseTop.activeInHierarchy)DestroyImmediate(Row2BaseTop);
		...
		if (!Row3.activeInHierarchy){
			...
			if (!Row3BackWall.activeInHierarchy)DestroyImmediate(Row3BackWall);
			...
			if (!Row3Stairs.activeInHierarchy)DestroyImmediate(Row3Stairs);
			if (!Row3BaseMiddle.activeInHierarchy)DestroyImmediate(Row3BaseMiddle);
			if (!Row3BaseBottom.activeInHierarchy)DestroyImmediate(Row3BaseBottom);
			if (!Row3BaseTop.activeInHierarchy)DestroyImmediate(Row3BaseTop);
			if (!Row3Tunnel.activeInHierarchy)DestroyImmediate(Row3Tunnel);
		...
		if (!BaseFloor.activeInHierarchy)DestroyImmediate(BaseFloor);
		if (!StairsEntrance.activeInHierarchy)DestroyImmediate(StairsEntrance);
	...
	void SetAutoStairs(){
			...
			var countAll = 0;
			while (countAll < LookLeft.Length){
				if(LookLeft[countAll].transform.name == "QuarterPart"){
					stairs = 1;
				}
				countAll +=1;
			}
			countAll = 0;
			while (countAll < LookRight.Length){
				if(LookRight[countAll].transform.name == "QuarterPart"){
					stairs = 1;
				}
				countAll +=1;
			}
			...
			if(stairs == 1){
				...
				Row2BackWall.SetActive(false);
				Row2BaseBottom.SetActive(false);
				Row2BaseTop.SetActive(false);
				...
				Row3BackWall.SetActive(false);
				Row3BaseBottom.SetActive(false);
				Row3BaseTop.SetActive(false);
				...
				//no stairs
				BackWallEntrance.SetActive(false);
				StairsEntrance.SetActive(false);
				BaseFloor.SetActive(true);
				
				Row2BackWallEntrance.SetActive(false);
				Row2Tunnel.SetActive(false);
				Row2Stairs.SetActive(false);
			
				Row3BackWallEntrance.SetActive(false);
				Row3Tunnel.SetActive(false);
				Row3Stairs.SetActive(false);
				Row3BaseMiddle.SetActive(false);
