...
public class RaptorAttack : MonoBehaviour
{
	...
	void Awake ()
	{
		//raptor = GameObject.FindGameObjectWithTag("Raptor").transform;
		//player = GameObject.FindGameObjectWithTag ("MainCamera");
		//playerHealth = player.GetComponent <PlayerHealth> ();
		//enemyHealth = GetComponent<EnemyHealth>();
		anim = GetComponent <Animator> ();
		raptorMovement = GetComponent <RaptorMovement> ();
		timer = 0.0f;
		playerHealth = player.GetComponent<PlayerHealth> ();
	...
	void Update ()
	{
		...
		//raptor.rotation = Quaternion.Slerp(raptor.rotation, Quaternion.Euler(90, 0, 0), Time.time);
		//		if (timer >= timeBetweenAttacks && playerInRange/* && enemyHealth.currentHealth > 0*/)


		if (playerInRange && raptorMovement.RaptorSeesPlayer && !playerHealth.isDead)
		{
			Attack ();
			//Apply eat animation
		...
		if (attacking) 
		{
			if (timer >= timeBetweenAttacks) 
			{
				...
				//				Debug.Log ("raptor finish attacking");
				//				Debug.Log (timer);
			}
		}
		...
		//if (attacking)
		//{
		//    raptor.rotation = Quaternion.Slerp(raptor.rotation, Quaternion.Euler(33, 247, 359), 1); //Time.deltaTime * 10
		//}
	...
	void Attack ()
	{
		...
		if (attacking) 
		{
			//Debug.Log ("raptor is already attacking");
			return;
		}
		...
		//Commenting out for Alpha
		//        Debug.Log("raptor begin Attackig");
		attacking = true;
