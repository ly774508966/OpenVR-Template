...
public class enemyAIDectection3 : MonoBehaviour {
    ...
    void OnTriggerStay(Collider target)
    {
        if (target.gameObject.tag == "Player")
        {
            ...
            {
                this.gameObject.GetComponentInParent<enemyKnightAIVerSimpler>().playerSpotted = true;
                this.gameObject.GetComponentInParent<enemyKnightAIVerSimpler>().lastPlayerPos = target.transform.position;
            }
        }
    ...
    {
        if (target.gameObject.tag == "Player")
            this.gameObject.GetComponentInParent<enemyKnightAIVerSimpler>().playerDetected = false;
    }
