...
public class castingManagerDeprecated : MonoBehaviour {
    ...
    void Cast()
    {
        ...
        if (castPrefab != null)
        {
            ...
            if (magic.GetComponent<ParticleSystem>())
            {
                ...
                initialRate = magic.transform.GetComponent<ParticleSystem>().emission.rateOverTime.constant;
            }
        }
    ...
    void castUpdater()
    {
        ...
        if (currentCast.GetComponent<ParticleSystem>() != null)
        {
            ...
            {
                magicParticleSystemExtension.SetEmissionRate(currentCast.GetComponent<ParticleSystem>(), initialRate * 0 * Mathf.Log(timer, spellTime));
                currentCast.GetComponent<ParticleSystem>().startSize = initialSize * Mathf.Log(timer, spellTime);
                currentCast.GetComponentInChildren<magicCastUpdater>().Timer = 0;
                for (int i = 0; i < currentCast.transform.childCount; i++)
                {
                    if (currentCast.transform.GetChild(i).GetComponent<ParticleSystem>())
                    {
                        //magicParticleSystemExtension.SetEmissionRate(currentCast.transform.GetChild(i).GetComponent<ParticleSystem>(), 0);
                        currentCast.transform.GetChild(i).GetComponent<ParticleSystem>().SetEmissionRate(0);
                    }
                }
            }
        }
