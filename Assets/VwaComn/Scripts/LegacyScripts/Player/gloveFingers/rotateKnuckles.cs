...
public class rotateKnuckles : MonoBehaviour {
    ...
    void LateUpdate ()
    {
        ...
        //float distance = transform.InverseTransformPoint(intermediateJoint.position).z;
        //if (intermediateJoint.position.z > maxDistalJointDist)
        //    maxDistalJointDist = transform.InverseTransformPoint(intermediateJoint.position).z;

        //float distance = (transform.InverseTransformPoint(intermediateJoint.position).z - (transform.InverseTransformPoint(maxDistance.bounds.max)).z);
        //distance = maxDistalJointDist - transform.InverseTransformPoint(intermediateJoint.position).z;

        //if (positionalRef.transform.InverseTransformPoint(intermediateJoint.position).z != olddist)
        //    Debug.LogWarning(positionalRef.transform.InverseTransformPoint(intermediateJoint.position).z);
        //olddist = positionalRef.transform.InverseTransformPoint(intermediateJoint.position).z;

        //if (yAxis < transform.InverseTransformPoint(proximalJoint.position).z)
        //{
        //proximalAngle = 90 * distance / maxDistalJointDist;

        //    transform.localRotation = Quaternion.AngleAxis(proximalAngle, Vector3.down);
        //}
    }
}
