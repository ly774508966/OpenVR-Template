...
public class enemyOgreBossAI : MonoBehaviour
{
    ...
    public bool amSmacked = false;
    public float smackedSpeed = 1000f;
	...
	private BarrierLogic BarrierLogicScript;

    void Awake()
    {
        thisAnimator = GetComponent<Animator>();
		BarrierLogicScript = (BarrierLogic) GetComponent<BarrierLogic> ();
    }
    ...
    {
        if (other.gameObject.tag == "Glove" || other.gameObject.tag == "castingManager" && amServer)
        {
			if (!BarrierLogicScript.isBarrierActive) {
				//thisAnimator.Stop();
				transform.GetComponent<Rigidbody> ().AddForce (-1 * (other.transform.position - transform.position * smackedSpeed));
				isHit = true;
			}
        }
    }
    ...
    void knightAIState(int state, int attackState, bool pieceOShitSpotted, bool shouldAttack, bool amAttacking, bool beingHit)
    {
        ...
        switch (state)
        {
            ...
            case 1:
                ...
                if (amServer)
                {
                    //There will be an edge case if there are null waypoints for it to go to... and never set targetpos.. maybe zero
                    //likely to be from scene object placed enemies
                    ...
                    else if (shouldAttack)
                    {
						...
						this.transform.LookAt (attackWaypoints[Random.Range(0, attackWaypoints.Length)].position);
            ...
            case 2:
                ...
                else if (shouldAttack)
                {
					...
					if (isAttacking) { 
						...
						attackState = 1;//Random.Range(1, 2);
					}
                    else if (!isAttacking)
                    {
                        attackState = 0;
            ...
            case 4:
                ...
                if (amServer && thisAnimator.GetInteger("generalState") != 10)
                {
                    var targetID = this.gameObject.GetComponent<PhotonView>();
    ...
    void damaged(bool beingHit)
    {
        ...
        thisAnimator.SetTrigger("damaged");
        if (thisAnimator.IsInTransition(0))
        {
